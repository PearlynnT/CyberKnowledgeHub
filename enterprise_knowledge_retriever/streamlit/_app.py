...
def initialize_document_retrieval(prod_mode: bool) -> Optional[DocumentRetrieval]:
    if prod_mode:
        pinata_api_key = st.session_state.PINATA_API_KEY
        pinata_secret_api_key = st.session_state.PINATA_SECRET_API_KEY
    else:
        pinata_api_key = os.environ.get('PINATA_API_KEY') or st.session_state.get('PINATA_API_KEY')
        pinata_secret_api_key = os.environ.get('PINATA_SECRET_API_KEY') or st.session_state.get('PINATA_SECRET_API_KEY')

    if are_credentials_set():
        try:
            return DocumentRetrieval(pinata_api_key=pinata_api_key, pinata_secret_api_key=pinata_secret_api_key)
        except Exception as e:
    ...
    with st.sidebar:
        ...
        if are_credentials_set():
            if st.session_state.document_retrieval is None:
                ...
                if st.session_state.document_retrieval is None:
                    st.error("Document retrieval failed to initialize. Please check your credentials.")
                    return
                logging.info(f"Document Retrieval: {st.session_state.document_retrieval}")
